...
[Ernst Kummer](https://en.wikipedia.org/wiki/Ernst_Kummer) extended this and proved the theorem for *all regular primes*, leaving irregular primes to be analyzed individually. Building on Kummer's work and using sophisticated computer studies (probably a big for loop tbh), other mathematicians were able to extend the proof to cover all prime exponents up to *four million*, but a proof for all exponents was inaccessible (meaning that it was either impossible, exceedingly difficult, or unachievable with current knowledge).
